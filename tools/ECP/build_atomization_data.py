...
sys.path.insert(0, str(Path(__file__).parent.parent.parent / 'reference_data'))
...
def merge_with_corrections(reference_df: pd.DataFrame,
    ...
    if ccsd_csv_path is None:
        ccsd_csv_path = Path(__file__).parent.parent.parent / 'data' / 'ECP' / 'ccsd_energies.csv'
    ...
    if data_dir is None:
        data_dir = Path(__file__).parent.parent.parent / 'reference_data'
...
def main():
    ...
    parser.add_argument('--data-dir', type=Path,
                        default=Path(__file__).parent.parent.parent / 'reference_data',
                        help='Directory containing reference CSV files')
    parser.add_argument('--ccsd-csv', type=Path, default=None,
                        help='Path to ccsd_energies.csv (default: data/ECP/ccsd_energies.csv)')
    parser.add_argument('--output', type=Path, default=None,
                        help='Output CSV file (default: atomization_comparison.csv in data-dir)')
    args = parser.parse_args()
    
    data_dir = args.data_dir
    output_csv = args.output or (Path(__file__).parent.parent.parent / 'data' / 'ECP' / 'atomization_comparison.csv')
    ccsd_csv_path = args.ccsd_csv or (Path(__file__).parent.parent.parent  / 'data' / 'ECP' / 'ccsd_energies.csv')
